...
2. The tool returns JSON of the form:
   ...
   ```
   `degraded` only appears when one engine failed but the other succeeded. If BOTH engines fail, the response is `{ "error": "both engines failed", "details": [...] }` — tell the user research is temporarily unavailable and stop.
